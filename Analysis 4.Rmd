...
DATA2 = na.omit(DATA) %>% crossv_kfold(k=10) %>% select(train, test)
...
for(i in 1:7){
  for (j in 1:7){
    ...
    OUT.RMSE[i,j] = RMSE.func(actual = DATA3.PREDICT$W, predict =
                                DATA3.PREDICT$.fitted)
  }
}
...
In the code chunk below, we start by making the information found the summarized information in `OUT.RMSE` tidy. There are three columns in `OUT.RMSE2` that links the cross-validated RMSE to all considered combinations of $I$ and $J$. Change `eval=T` to `eval=T` before knitting to HTML.
...
BEST5.RMSE = as.tibble(top_n(OUT.RMSE2, -5, RMSE)) %>% arrange(-desc(RMSE))
...
Now, observe the figure below that shows the change in maximum water temperature ($W$) across Julian days ($D$). Change `eval=T` to `eval=T` before knitting to HTML.
...
BEST5.DATA2 = BEST5.DATA %>% gather(key = "Model", "First", "Second", "Third", "Fourth", "Fifth", value = "Predict", factor_key=T)
BEST5.DATA2
...
ggplot(BEST5.DATA2) + 
  geom_point(aes(x = D, y = W), alpha = 0.05, stroke = 0) +
  geom_line(aes(x = D, y = Predict, color = Model)) + xlab("Julian Day") + ylab("Max Water Temperature") 



```

Answer *(2 Points)*: The predicted maximum water temperatures differ for the top five models because, based on the graph above, the range for the fourth and fifth model appears to be higher than the range for the other three models (and the range for the fifth is the highest). 


###Q4 *(3 Points)*

The following two figures show the marginal change in the average out-of-sample RMSE as $I$ and $J$ increase. Based on these figures, what $I$ and $J$ would you recommend going forward? Critically, think about what these figures are telling us. Give a reason for your answer based on these graphics. Answer the question below the two figures in complete sentences. Change `eval=T` to `eval=T` before knitting to HTML.
...
Answer: I would recommend an 'I' value of 4 and a 'J' value of 7. The reason why is because the average RMSE value at these points is the lowest in comparison to the other values along the x-axis. The reason why a lower average RMSE value is optimal is because it indicates a better fit. In other words, a lower RMSE indicates that a model more accurately predicts the response.
...
BEST.func=function(data){
  row = which.min(data$RMSE)
  A = data[row,1]
  B = data[row,2]
  C = as.vector(c(as.numeric(A), as.numeric(B)))
  return(C)
}
...
ggplot(DATA) + geom_point(aes(x = W, y = Predict), alpha = 0.15, stroke = 0) + geom_abline(intercept = 0, slope = 1, color = "red") + xlab("Actual Max Water Temperature") + ylab("Fitted Max Water Temperature") 
```
...
ggplot(DATA) + geom_point(aes(x = D, y = Predict), alpha = 0.15, stroke = 0) + geom_abline(intercept = 0, slope = 0, color = "red") + xlab("Day") + ylab("Residual")
```